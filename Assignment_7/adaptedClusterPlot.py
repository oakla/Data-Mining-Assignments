...
def clusterPlot(X, clusterid, centroids=None, y=None):
	...
	class_labels = np.unique(np.concatenate(preds,y_train))
    # K = np.size(np.unique(cls))
    # C = np.size(np.unique(y))
    ncolors = np.size(class_labels)
	...
	#Orignal: go through all true class labels and plot the corresponding x
	#Adapted: go through all y_train and plot the corresponding x_train
    ...
    for i, cr in enumerate(np.unique(preds)):
        plt.plot(X_test[(preds == cr).ravel(), 0], X_test[(preds == cr).ravel(), 1], 'o',
